...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        #print("B,N,C",B,N,C) #B batchsize N 129 C 768
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        patch_attention_matrix3 = torch.ones(1,242 ,768)
        self.patch_attention_matrix3 = nn.Parameter(patch_attention_matrix3)
    def forward(self, x):
        if not self.use_depthwise:
            batch_size, num_token, embed_dim = x.shape
            #print(x.shape)  [48,243,768]
            #x = self.transpool(x)
            ...
            #x = transpool1(x)
            #print(x.shape)
            #if num_token==61 or num_token==81:
            #    x = x +self.drop_path(self.attn(self.norm1(x)))
            #    x = x +self.drop_path(self.mlp(self.norm2(x)))
                ...
                #x = x.view(batch_size,y1, x1, embed_dim).permute(0, 3, 1, 2)
                #print(x)
                #x2 =x[:,:,:,10:11]
            ...
            #    cls_token, x = torch.split(x, [1, num_token - 1], dim=1)
            #    x = x.mul(self.patch_attention_matrix3)
            #    x = torch.cat([cls_token, x], dim=1)
            return x
        else:
            batch_size, num_token, embed_dim = x.shape
...
class transpool(nn.Module):
    def __init__(self):
        super().__init__()
        #self.maxpool = nn.AdaptiveMaxPool2d((11,10))
        #self.sigmoid = nn.Sigmoid()
        #self.maxpool = nn.MaxPool2d((1,2),1)

        #self.sigmod = nn.ReLU()
    def forward(self,x):

        batch,num_token,embed_dim = x.shape
        #print(x.shape)
        for i in range(0,num_token-1,2):
            x[:,i+1]= (x[:,i+1]+x[:,i+2])/2
            x[:,i+2]=x[:,i+1]
        return x
...
def transpool1(x):
    batch,num_token,embed_dim=x.shape
    #class_token = x[:,0:1]
    #x2 = torch.cat([x[:,2:num_token],x[:,num_token-1:num_token]],dim=1)
    #x3 = x[:,1:num_token].add(x2)/2
    #x =torch.cat([class_token,x3],dim=1)
    cls_token, x = torch.split(x, [1, num_token - 1], dim=1)
    x = x.view(batch,y1, x1, embed_dim).permute(0, 3, 1, 2)
    print(x.shape)
    for i in range(0,num_token-1,2):
        x[:,i+1] = (x[:,i+1]+x[:,i+2])/2
        x[:,i+2] = x[:,i+1]
...
class PatchEmbed_overlap(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=768):
        ...
        print("to_2tuple(img_size)",img_size)
        patch_size = to_2tuple(patch_size) #[16,16]
        print("to2tuple",patch_size)
        print("stride_size",stride_size)
        stride_size_tuple = to_2tuple(stride_size)
        #print(stride_size_tuple)
        ...
        self.patch_size = patch_size
        self.num_patches = num_patches
        #self.proj1 = nn.Conv2d(in_chans,in_chans,kernel_size=3,padding=1,stride=1)
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride_size)
        #self.stage = nn.Sequential(
        #    nn.Conv2d(in_chans, in_chans, kernel_size=3, padding=1, stride=1),
        #    nn.BatchNorm2d(in_chans),
        #    nn.ReLU(True),
        #)
        #self.shortcut = nn.Conv2d(in_chans,in_chans,kernel_size=3,padding=1,stride=1)
    ...
    def forward(self, x):
        ...
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x)
        #print("卷积后的x.shape:",x.shape)
        x = x.flatten(2).transpose(1, 2) # [64, 128, 768]
        #print("进一步调整x",x.shape)
        return x
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0.2, attn_drop_rate=0., camera=0, view=0,
        ...
        self.pos_drop = nn.Dropout(p=drop_rate) #dropout防止过拟合
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule

        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,use_depthwise=False)
        ...
        patch_attention_matrix = torch.ones(1,242,self.embed_dim)
        self.patch_attention_matrix = nn.Parameter(patch_attention_matrix)
    ...
    def compute_similarity(self,x):
        list1 = [1 for _ in range(242)]
        y = torch.split(x,list1,dim=1)  #len(y)=22
        print(y[0].shape)
        sim= []
        maxsim = 0
        tag =0
        for (index,state) in enumerate(y):
            for i in range(index+1,22):
                x1 = F.cosine_similarity(x[:,index:index+1],x[:,i:i+1],2)
                sim.append(x1)
                #print(state)
        print(len(sim))
        guodu =x
        ...
        for i in range(1,231):
            #print(i)
            x = torch.cat([x,sim[i].unsqueeze(1)],dim=2)
        print(x)   #[48,1,231]
        max1 ,index = x.min(2)
        print(max1)
        print(index)
    ...
    def forward_features(self, x, camera_id, view_id):
        B = x.shape[0]
        #print("B",B)
        #print("patch overlop之前",x.shape)  #[batch ,3,256,768]
        x = self.patch_embed(x)
        #print("patch overlop之后",x.shape) #[batchszie,128,768]
        x = x.mul(self.patch_attention_matrix)

        #x[:,220:242]=x[:,60:82]
        #x[:,0:20] = x[:,20:40]
        #print(self.patch_attention_matrix)
        #x = x * self.patch_attention_matrix
        #print(x.shape)
        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        #print("cls_tokens.shape",cls_tokens.shape) #[batchsize,1,768]
        x = torch.cat((cls_tokens, x), dim=1)
        #print("融合cls之后的x",x.shape)   #[batchsize,129,768]
        if self.cam_num > 0 and self.view_num > 0:
            ...
            #print("走4")
            #print("posembed shape",self.pos_embed.shape)  [1,129,768]
            x = x + self.pos_embed
        #print("融合positionembeed+sie",x.shape) #[batchsize 129 768]
        x = self.pos_drop(x)
        #print("hello1")
        #print(x.shape)
        #print("pos_drop之后",x.shape) #[batchsize,129,768]
        #print("enter block")
        #print(self.local_feature) #false
        #print(self.blocks[:-1])
...
def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
    model = TransReID(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, num_heads=12, mlp_ratio=4, qkv_bias=True,\
